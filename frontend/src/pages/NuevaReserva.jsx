...
const iconos = {
  'habitación estándar': '🛏️', 'suite': '👑', 'restaurante': '🍽️',
  'spa': '💆', 'masaje': '💆', 'piscina': '🏊', 'sala': '🏢',
  'estacionamiento': '🚗', 'consulta': '🩺', 'tenis': '🎾',
};
...
export default function NuevaReserva() {
  ...
  const servicioSel = servicios.find(s => String(s.id) === servicioId);

  useEffect(() => { api.getServicios().then(setServicios).catch(() => {}); }, []);
  ...
  if (success) return (
    <div>
      <div className="page-header"><h2>¡Reserva confirmada!</h2></div>
      <div className="card" style={{ maxWidth: 480 }}>
        <div className="card-body" style={{ textAlign: 'center', padding: '40px 24px' }}>
          <div style={{ fontSize: '3rem', marginBottom: 16 }}>✅</div>
          <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem', marginBottom: 8 }}>Tu reserva ha sido registrada</h3>
          <p style={{ color: 'var(--text-secondary)', marginBottom: 24, fontSize: '0.9rem' }}>Recibirás una notificación de confirmación.</p>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <button className="btn btn-secondary" onClick={() => { setSuccess(false); setStep(1); setServicioId(''); setFecha(''); setSlotId(''); }}>Nueva reserva</button>
    ...
    <div>
      ...
      <div className="card" style={{ maxWidth: 700 }}>
        <div className="card-body">
          ...
          {/* Step 1 — grilla de servicios */}
          {step === 1 && (
            <div>
              <h3 style={{ fontFamily: 'var(--font-display)', marginBottom: 20 }}>¿Qué servicio necesitas?</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: 12 }}>
                {servicios.map(s => (
                  <button key={s.id} onClick={() => setServicioId(String(s.id))} style={{
                    padding: '20px 12px',
                    border: `2px solid ${servicioId === String(s.id) ? 'var(--accent)' : 'var(--border)'}`,
                    borderRadius: 'var(--radius)',
                    background: servicioId === String(s.id) ? 'var(--accent-light)' : 'var(--bg)',
                    cursor: 'pointer', textAlign: 'center', transition: 'all 0.15s',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
                  }}>
                    <span style={{ fontSize: '2rem' }}>{getIcono(s.nombre)}</span>
                    <span style={{ fontWeight: 600, fontSize: '0.82rem', color: 'var(--text-primary)', lineHeight: 1.3 }}>{s.nombre}</span>
                    <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{s.duracion_minutos} min</span>
                  </button>
                ))}
              </div>
              <div style={{ marginTop: 24, display: 'flex', justifyContent: 'flex-end' }}>
                <button className="btn btn-primary" disabled={!servicioId} onClick={() => setStep(2)}>Continuar →</button>
              </div>
            </div>
          )}

          {/* Step 2 — fecha y slots */}
          {step === 2 && (
            <div>
              <h3 style={{ fontFamily: 'var(--font-display)', marginBottom: 4 }}>Elige fecha y horario</h3>
              {servicioSel && (
                <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginBottom: 20 }}>
                  {getIcono(servicioSel.nombre)} <strong>{servicioSel.nombre}</strong> · {servicioSel.duracion_minutos} min
                </p>
              )}
              <div className="form-group">
                <label className="form-label">Fecha</label>
                <input className="form-input" type="date" min={today} max={maxDate} value={fecha}
                  onChange={e => setFecha(e.target.value)} style={{ maxWidth: 220 }} />
              </div>
              {fecha && (
                <div>
                  <label className="form-label" style={{ marginBottom: 12 }}>Horarios disponibles</label>
                  {slots.length === 0
                    ? <div className="empty-state" style={{ padding: '24px 0' }}><p>No hay horarios disponibles para esta fecha</p></div>
                    : <div className="slots-grid">
                        {slots.map(s => (
                          <div key={s.id} className={`slot ${slotId === String(s.id) ? 'selected' : ''}`} onClick={() => setSlotId(String(s.id))}>
              ...
              <div>
                ...
                <div style={{ background: 'var(--bg)', borderRadius: 'var(--radius-sm)', padding: 20, marginBottom: 20 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 24px' }}>
                    {[
                      ['Servicio', `${getIcono(servicioSel?.nombre)} ${servicioSel?.nombre}`],
                      ['Fecha', fecha ? format(new Date(fecha + 'T12:00:00'), "d 'de' MMMM yyyy", { locale: es }) : ''],
                      ['Horario', slot ? `${slot.hora_inicio?.slice(0,5)} – ${slot.hora_fin?.slice(0,5)}` : ''],
                      ['Duración', `${servicioSel?.duracion_minutos} min`],
                    ].map(([label, val]) => (
                      <div key={label}>
                ...
                <div className="form-group">
                  ...
                  <textarea className="form-input" rows={3} placeholder="Información adicional..." value={notas}
                    onChange={e => setNotas(e.target.value)} style={{ resize: 'vertical' }} />
                </div>
